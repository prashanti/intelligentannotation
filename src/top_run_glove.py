...
parser.add_argument(
    "-v", "--glove_vector", type=str, default='840B_300d',
    choices=[
        '6B_50d', '6B_100d', '6B_200d', '66B_300d',
        '42B_300d', '840B_300d', 'twitter_25d',
        'twitter_50d', 'twitter_100d', 'twitter_200d'],
    help="Pretrained GloVe word vector")
...
if config.get('new_log'):
    with open(os.path.join(LOG_DIR, "glove.log"), 'w+') as f:
        pass


file_handler = logging.FileHandler(filename=os.path.join(LOG_DIR, "glove.log"))
...
files_to_remove = []
glove_filename = config.get('glove_vector')
glove_dim = glove_filename.split("_")[-1]
if "6B" in glove_filename:
    FILENAME = "glove.6B.zip"
    input_file = os.path.join(
        EMB_DIR,
        FILENAME.replace(
            "zip", glove_dim + ".txt"))
    for i in ['50d', '100d', '200d', '300d']:
        if i != glove_dim:
            files_to_remove.append(
                input_file.replace(glove_dim, i))
elif glove_filename == "42B_300d":
    FILENAME = "glove.42B.300d.zip"
    input_file = os.path.join(
        EMB_DIR,
        FILENAME.replace("zip", "txt"))
elif glove_filename == "840B_300d":
    ...
    input_file = os.path.join(
        EMB_DIR,
        FILENAME.replace(
            "zip", glove_filename.split("_")[-1] + ".txt"))
    for i in ['25d', '50d', '100d', '200d']:
        if i != glove_dim:
            files_to_remove.append(
                input_file.replace(glove_dim, i))

files_to_remove.append(input_file)
BIN_FILENAME = input_file.replace("txt", "bin")

os.environ['EMB_DIR'] = EMB_DIR
os.environ["FILENAME"] = FILENAME

# ## If pretrained glove vector is not available,
# download the vector file
if not os.path.exists(BIN_FILENAME):
    to_log('GloVe vector bin file not found')
    if not os.path.exists(BIN_FILENAME.replace("bin", "zip")):
        to_log(
            'GloVe vector zip file not found'
            '\nDownloading zip file from huggingface')
        # ## Download
        os.system(
            "wget https://huggingface.co/stanfordnlp/glove/"
            "resolve/main/$FILENAME")
    # ## Unzip and remove zip file
    to_log('Unzipping zip file')
    os.system('unzip $FILENAME -d $EMB_DIR')
    to_log('Removing zip file')
    os.system('rm $FILENAME')

    # ## Convert text to bin for lower memory consumption
    # input_file = os.path.join(EMB_DIR, FILENAME.replace("zip", "txt"))
    to_log('Converting glove to word2vec')
    output_file = os.path.join(EMB_DIR, "word2vec.txt")
    glove2word2vec(input_file, output_file)

    to_log('Saving word2vec as binary file')
    model = KeyedVectors.load_word2vec_format(output_file, binary=False)
    model.save_word2vec_format(BIN_FILENAME, binary=True)

    to_log('Removing temporary files')
...
if __name__ == '__main__':
    ...
    config.update({
        ...
        "extra_info": "GloVe, inputs: Word(30D), POS(100D)",
        "name": "GLOVE",
    })
    ...
    to_log('Creating initialization vector for Glove Embedding')
    word_to_vec = KeyedVectors.load_word2vec_format(BIN_FILENAME, binary=True)
    # wv_vocab = word_to_vec.index_to_key
    # wv_vector = [word_to_vec.get_vector(i) for i in wv_vocab]
    # embeddings_index = dict(
    #     (vocab, vector) for vocab, vector in zip(wv_vocab, wv_vector)
    # )
    emb_matrix = np.zeros((n_words, int(glove_dim.replace("d", ""))))
    hits, misses = 0, 0
    for word, index in word_to_idx.items():
        try:
            vec = word_to_vec.get_vector(word)
    ...
    emb_word = Embedding(
        input_dim=n_words, output_dim=int(glove_dim.replace("d", "")),
        embeddings_initializer=tf.keras.initializers.Constant(emb_matrix),
        input_length=config.get("max_len"), trainable=True,
        name="GloVe")(word_in)
    ...
    model = Model([word_in, char_in, pos_in], out, name="GLOVE")
    model.compile(
    ...
    model_arch = tf.keras.utils.plot_model(
        model, to_file='glove_architecture.png', show_shapes=True)
