...
GLOVE_EMB_FILE = os.path.join(DATA_DIR, "Embedding", "glove.840B.300d.bin")
...
if __name__ == '__main__':
    ...
    logger.info('Creating initialization vector for Glove Embedding')
    print('Creating initialization vector for Glove Embedding')
    ...
    embeddings_index = dict(
        (vocab, vector) for vocab, vector in zip(w2v_vocab, w2v_vector)
    )
    ...
    emb_word = Embedding(
        input_dim=n_words, output_dim=300,
    ...
    for i in range(int(len(X_te)/step)):
